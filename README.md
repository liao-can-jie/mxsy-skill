...
| [`mxsy-pre-cabinet-door`](./skills/mxsy-pre-cabinet-door/SKILL.md) | 秒享商用换电测试/生产环境电柜门操控技能。用于构造和调用 `POST /mx-ce-system/api/cabinet/optCabinetDoor`，支持打开一个或多个仓门、禁用/启用一个或多个仓门、禁用/启用整柜，并要求从安全配置或明确用户输入获取操作密码。包含渐进式限流规则和完整的业务错误码。 |
| [`mxsy-pre-system-service`](./skills/mxsy-pre-system-service/SKILL.md) | 秒享商用换电测试/生产环境系统服务聚合技能。基于 `系统服务.md` 生成接口索引与参数明细，覆盖区域、用户、角色、菜单、部门、字典、仓储、电池、电柜、工单、企业、订单、活动等系统服务接口（共 501 个 API）；已排除认证登录与电柜门操控等已有专用技能覆盖的接口。 |
| [`mxsy-cabinet`](./skills/mxsy-cabinet/SKILL.md) | 秒享商用换电电柜操作实用技能集。包含电柜详情、柜仓信息、PC/移动端操作日志、柜效查询、满电阀值调节、音量调节、告警查询、仓门管理、远程控制、PC端导出、PC端编辑、地图功能等 30+ 常用电柜管理接口。适用于日常电柜运维操作。 |
| [`mxsy-ops-playbook`](./skills/mxsy-ops-playbook/SKILL.md) | 秒享商用换电双环境运维手册。覆盖认证登录、电柜操作、电池管理、工单处理、运营统计、用户管理、套餐管理、押金管理、仓门管理、告警处理等 500+ API 索引、完整踩坑记录、限流规则说明和常见错误码速查表。 |
...
```txt
使用 mxsy-cabinet，帮我查询电柜 TSD25100059 的柜仓信息和操作日志。
```

```txt
使用 mxsy-ops-playbook，帮我查看电池远程控制的接口和常见错误码。
```
...
Copy-Item -Recurse -Force .\skills\mxsy-cabinet $skillsDir
Copy-Item -Recurse -Force .\skills\mxsy-ops-playbook $skillsDir
```
...
```txt
使用 mxsy-pre-cabinet-door，打开电柜 TSD25100059 的所有仓门。
```
...
```txt
使用 mxsy-cabinet，查询电柜 TSD25100059 的详细信息和柜仓状态。
```

对应接口：

```
GET /mx-ce-system/api/cabinet/cabinetDetail?cabinetCode=TSD25100059
GET /mx-ce-system/api/cabinet/cabinetDoorInfo?cabinetCode=TSD25100059
```

### 查询柜效和操作日志

```txt
使用 mxsy-cabinet，帮我查询电柜 TSD25100059 的柜效数据和最近的操作日志。
```

### 调节满电阀值

```txt
使用 mxsy-cabinet，将电柜 TSD25100059 的满电阀值调整为 60%。
```
...
| 开门 | `optType=1`，指定仓门时传 `doorIds`；打开所有仓门时传 `null` 或空数组 |
| 禁用 | `optType=2`，`doorIds` 为空或不传表示整柜禁用 |
| 启用 | `optType=3`，`doorIds` 为空或不传表示整柜启用 |
| 多仓门操作 | `doorIds` 传数组，例如 `[1, 2, 3]` |
| 仓门操作限流 | 渐进式限流，Level 1: 5次/10秒 → Level 4: 40次/150秒 |
...
python C:\Users\Administrator\.codex\skills\.system\skill-creator\scripts\quick_validate.py .\skills\mxsy-cabinet
python C:\Users\Administrator\.codex\skills\.system\skill-creator\scripts\quick_validate.py .\skills\mxsy-ops-playbook
```
